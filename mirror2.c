...
void msg_to_client(int sock, const char* message) {
...
//dirlist -a
struct DirEntry *insertSorted(struct DirEntry *head, const char *name) {     //dirlist -a
    struct DirEntry *newNode = (struct DirEntry *)malloc(sizeof(struct DirEntry));
    if (!newNode) {
...
//dirlist -t
struct DirEntryTime *insertSortedByTime(struct DirEntryTime *head, const char *name, time_t creation_time) {
    struct DirEntryTime *newNode = (struct DirEntryTime *)malloc(sizeof(struct DirEntryTime));
    if (!newNode) {
        perror("Failed to allocate memory for new directory entry");
        return head;
    }
...
void list_subdirectories(int sock, const char *directory) {
    ...
    while ((dir = readdir(d)) != NULL) {
        if (dir->d_type == DT_DIR && strcmp(dir->d_name, ".") != 0 && strcmp(dir->d_name, "..") != 0) {
            head = insertSorted(head, dir->d_name);
        }
    }
    closedir(d);
...
void list_subdirectories_by_time(int sock, const char *directory) {
  ...
  while ((dir = readdir(d)) != NULL) {
    if (dir->d_type == DT_DIR && strcmp(dir->d_name, ".") != 0 && strcmp(dir->d_name, "..") != 0) {
        ...
        if (syscall(__NR_statx, AT_FDCWD, full_path, AT_SYMLINK_NOFOLLOW, STATX_BTIME, &buf) == 0) {
            head = insertSortedByTime(head, dir->d_name, buf.stx_btime.tv_sec);  // Use birth time
        } else {
            perror("statx failed");
            // Optional: Fallback to ctime or mtime if statx fails
            struct stat old_stat;
            stat(full_path, &old_stat);
            head = insertSortedByTime(head, dir->d_name, old_stat.st_ctime);  // Fallback to change time
        }
    }
}
closedir(d);
...
void handle_dirlist_command(char* options, int sock) {
...
//w24fn
void w24fn_cmd_handle(int sock, const char *filename) {
    char command[512];
    FILE *fp;
